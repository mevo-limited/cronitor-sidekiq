...
module Sidekiq::Cronitor
  class PeriodicJobs
    def self.sync_schedule!
      ...
      loops.each do |lop|
        lop_options = lop.options.is_a?(String) ? JSON.parse(lop.options) : lop.options

        if lop_options.has_key?('cronitor_enabled') || lop.klass.constantize.sidekiq_options.has_key?('cronitor_enabled')
